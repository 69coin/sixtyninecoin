...
#if defined(NDEBUG)
# error "Sixtyninecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x996e48982f3cd9558b3dcb5402cc102277e3c06675e4e2b533190521e71c8990");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sixtyninecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sixtyninecoin Signed Message:\n";
...
{
    // Sixtyninecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 70656 * COIN;

    // Subsidy is cut in half every 10000 blocks, which will occur approximately every 35 years
    nSubsidy >>= (nHeight / 10000); // Sixtyninecoin: 10k blocks in ~35 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.125 * 24 * 60 * 60; // Sixtyninecoin: 0.125 days
static const int64 nTargetSpacing = 5 * 60; // Sixtyninecoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sixtyninecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sixtyninecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1d8996cdcd37125c4ea93bc66a7f23d0da3572ded1316adcea44785ebce07fd0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Yahoo News 03/Jul/2021 Elon Musk is losing power over the crypto community";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 70656 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044956a9c794f5173f8b205af2d85c5c9dead01379db167d98464bbb0d24ba712eed6060263166689b8b88a955c3339db8d96499a7fb5af880d8be53c6e3900b25") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1625595210;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084669748;

        if (fTestNet)
        {
            block.nTime    = 1625595194;
            block.nNonce   = 385450175;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa5d3c962d625dfe97f5656b15ff24be71d1e0156230cb8e3a3cbee1f933f2a73"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc1, 0xb3, 0xd3 }; // Sixtyninecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SixtyninecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SixtyninecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SixtyninecoinMiner(CWallet *pwallet)
{
    printf("SixtyninecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sixtyninecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SixtyninecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SixtyninecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SixtyninecoinMiner, pwallet));
}
